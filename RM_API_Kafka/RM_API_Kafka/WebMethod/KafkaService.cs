    ...
    string>(conf).Build())
            ...
            catch (Exception ex)
            {
                ...
                if (id != null && resList != null && resList.Count > 0)
                    return resList.Where(x => x.ResourceId == id).GroupBy(x => x.ResourceId).Select(x => x.LastOrDefault()).ToList<ResourceWithValue>();
                else if (resList != null && resList.Count > 0)
                    return resList.GroupBy(x => x.ResourceId).Select(x => x.LastOrDefault()).ToList<ResourceWithValue>();
            //catch(Exception ex)
            //{
